 ...
 * Also creates string pairs for training. 
 *
...
public class PairwiseSelector implements OptionHandler, Serializable {
  ...
  /** The number of actual same-class pairs from last call */
  protected int m_numActualPositives = 0;

  /** The number of actual different-class pairs from last call  */
  protected int m_numActualNegatives = 0;

  /** Output debugging information */
  protected boolean m_debug = false;

  /** Should equal strings be used? */
  protected boolean m_useEqualStrings = true;

  /** Should we require common tokens? */
  protected boolean m_requireCommonTokens = false; 
  ...
  public Instances getInstances(int [] attrIdxs, StringMetric[][] stringMetrics,
    ...
    m_numActualPositives = 0;
    m_numActualNegatives = 0;
	...
	if (trainInstance != null && isUniqueInstance(trainInstance, checksumMap, checksumCoeffs)) {
	  ...
	  if (trainInstance.value(trainInstance.numValues()-1) == 0) {
	    m_numActualPositives++;
	  } else {
	    m_numActualNegatives++;
	  }
	} else {
	  i--; 
	  numMisfires++;
    ...
    case POS_MODE_RANDOM_POSITIVES:
      ...
      numMisfires = 0; 
      int [] posPairIdxs = randomSubset(m_numPotentialPositives,
                                        m_numPotentialPositives);
      for (int i = 0;
           i < posPairIdxs.length && m_numActualPositives < numPosPairs
             && numMisfires < 500; i++) {
	Instance posInstance =
          createInstance((InstancePair) m_posPairList.get(posPairIdxs[i]),
    ...
    case POS_MODE_STATIC_ACTIVE:
      Blocking blocker = new BlockingOld();
      blocker.buildIndex(m_instances);
      InstancePair[] pairs = blocker.getMostSimilarPairs(numPosPairs*2);
      numMisfires = 0; 
      for (int i = 0;
	   (m_numActualPositives + m_numActualNegatives) < numPosPairs
             && i < pairs.length && pairs[i] != null && numMisfires < 500; i++) {
	...
	if (trainInstance != null && isUniqueInstance(trainInstance, checksumMap, checksumCoeffs)) {
      ...
      System.out.println("After static-active:\t" + m_numActualPositives + " positives and " +
			 m_numActualNegatives + " negatives");
    ...
    System.out.println("POSITIVES:  requested=" + numPosPairs
                       + "\tpossible=" + m_numPotentialPositives +
		       "\tactual=" + m_numActualPositives);
    System.out.println("NEGATIVES:  requested=" + numNegPairs
                       + "\tpossible=" + m_numPotentialNegatives +
		       "\tactual=" + m_numActualNegatives);
    return instances;
  }
  ...
  public ArrayList getStringPairList(Instances instances, int attrIdx,
    ...
    return getStringPairList(instances, attrIdx, 0, numPosPairs, 
                             0, numNegPairs, 0,  false, metric);
  }
   ...
   */
  public ArrayList getStringPairList(Instances instances, int attrIdx,
				     int numUnsegPosPairs, int numSegPosPairs,
                                     int numUnsegNegPairs, int numSegNegPairs,
                                     int numSegSamePairs, 
                                     boolean separateSegUnseg,
				     StringMetric metric) throws Exception {
    ...
    m_numActualPositives = 0;
    m_numActualNegatives = 0;
    int numPosPairs = numUnsegPosPairs + numSegPosPairs;
    int numNegPairs = numUnsegNegPairs + numSegNegPairs;
    ...
    case STRING_PAIRS_EASIEST:
      ...
      for (int i = 0; iterator.hasNext()
             && i < m_numPotentialPositives && i < numPosPairs; i++) {
	...
	if (!m_requireCommonTokens || haveCommonTokens(str1, str2)) {
          if (m_useEqualStrings || !str1.equals(str2)) { 
            StringPair strPair = new StringPair(str1, str2, true, 0);
            strPairList.add(strPair);
          } else {
            System.out.println("Not using equal strings: " + str1 + "\t" + str2);
          } 
	} else {
	  System.out.println("No common tokens - NOT ADDING: " + str1 + "\t" + str2);
	}
      } 
      ...
      if (numPossiblePosStrPairs <= numSegPosPairs+numUnsegPosPairs) {
        System.out.println("INSUFFICIENT POSITIVE examples "
                           + numPossiblePosStrPairs);        
        pairList = strPairList;
      } else {
        // if we have more than enough potential pairs, sample randomly with replacement
        int[] indexes = randomSubset(numSegPosPairs+numUnsegPosPairs,
                                     numPossiblePosStrPairs);
        System.out.println("SUFFICIENT  POSITIVE examples, randomly selected "
                           + indexes.length + " of " + numPossiblePosStrPairs);
        for (int i = 0; i < indexes.length; i++) {
          pairList.add(strPairList.get(indexes[i]));
        }
      }

      // drop a few segmentations if necessary
      if (separateSegUnseg) {
        int[] indexes = randomSubset(numUnsegPosPairs, pairList.size());
        for (int idx : indexes) {
          StringPair pair = (StringPair) pairList.get(idx);
          System.out.println("\t" + pair);
          pair.str1 = SegTextConverter.stripXML(pair.str1);
          pair.str2 = SegTextConverter.stripXML(pair.str2);
          //          System.out.println("\t" + pair);
        }
        System.out.println("Made " + numUnsegPosPairs + " unsegmented"); 
      } 

    

//         System.out.println("String[][] data = new String[" + strPairList.size() + "][2];");
//       for (int k = 0; k < strPairList.size(); k++) {
//         StringPair pair = (StringPair) strPairList.get(k);
//         System.out.println("data[" + k + "][0] = new String(\"" + pair.str1 + "\"); \t\tdata[" + k + "][1] = new String(\"" + pair.str2 + "\");");
//       }
//       System.out.println("Training data, positives:"); 
...
//       }
    ...
    m_numActualPositives = pairList.size();
	...
	if (strPairList.size() <= numNegPairs) {
	  ...
	  for (int i = 0; i < indexes.length; i++) {
        ...
        // drop a few segmentations if necessary
        if (separateSegUnseg) {
          int[] indexes = randomSubset(numUnsegNegPairs, pairList.size()-m_numActualPositives);
          for (int idx : indexes) {
            StringPair pair = (StringPair) pairList.get(idx+m_numActualPositives);
            System.out.println("\t" + pair);
            pair.str1 = SegTextConverter.stripXML(pair.str1);
            pair.str2 = SegTextConverter.stripXML(pair.str2);
            System.out.println("\t" + pair);
          }
          System.out.println("Made " + numUnsegNegPairs + " unsegmented negatives"); 
        } 
	break;
    ...
    m_numActualNegatives = pairList.size() - m_numActualPositives;


    
    // SegSame if required
    if (numSegSamePairs > 0) {
      Random r = new Random(numSegSamePairs);
      int numInstances = instances.numInstances();
      for (int i = 0; i < numSegSamePairs; i++) {
        int instanceIdx = r.nextInt(numInstances);
        Instance instance = instances.instance(instanceIdx);
        String str = instance.stringValue(attrIdx); 
        StringPair pair = new StringPair(str, str, true, 0);
        pairList.add(pair); 
      }
      System.out.println("Made " + numSegSamePairs + " same-string segmented pairs"); 
    } 

    System.out.println();
    System.out.println("**POSITIVES:  requested=" + numPosPairs
                       + "\tpossible=" + numPossiblePosStrPairs 
		       + "\tactual=" + m_numActualPositives);
    System.out.println("**NEGATIVES:  requested=" + numNegPairs
                       + "\tpossible=" + numPossibleNegStrPairs 
		       + "\tactual=" + m_numActualNegatives);
    return pairList;
  }
  ...
  /** Get the last number of actual positives */
  public int numActualPositives() { return m_numActualPositives; }

  /** Get the last number of actual negatives */
  public int numActualNegatives() { return m_numActualNegatives; }
  ...
  /** Turn on/off the use of equal strings */
  public void setUseEqualStrings(boolean use) {
    m_useEqualStrings = use;
  }
  public boolean getUseEqualStrings() { return m_useEqualStrings; }

  /** Turn on/off requiring positive strings to have common tokens */
  public void setRequireCommonTokens(boolean require) {
    m_requireCommonTokens = require;
  }
  public boolean getRequireCommonTokens() { return m_requireCommonTokens; }
  ...
  public String [] getOptions() {
    ...
    if (m_useEqualStrings) {
      options[current++] = "-useEq";
    } else {
      options[current++] = "-noEq";
    }

    if (m_requireCommonTokens) {
      options[current++] = "-reqComn";
    } 
