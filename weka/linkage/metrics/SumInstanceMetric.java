...
public class SumInstanceMetric extends InstanceMetric implements OptionHandler, Serializable {
  ...
  protected StringMetric m_metric = new AffineEditDistance();

  /** The number of positive pairs desired for training */
  protected int m_numPosPairs = 100;
  protected int m_numNegPairs = 0;

  protected int m_numUnsegPosPairs = 0;
  protected int m_numUnsegNegPairs = 0;

  protected int m_numSegSamePairs = 0;
  ...
  public void trainInstanceMetric(Instances trainData, Instances testData) throws Exception {
    ...
    if (m_metric instanceof DataDependentStringMetric) {
      for (int i = 0; i < m_stringMetrics.length; i++) {
	ArrayList<String> stringList = getStringList(trainData, testData, m_attrIdxs[i]);
	...
	ArrayList strPairList = m_selector.getStringPairList(trainData, m_attrIdxs[i],
							     m_numUnsegPosPairs, m_numPosPairs,
                                                             m_numUnsegNegPairs, m_numNegPairs,
                                                             m_numSegSamePairs,
                                                             (m_numUnsegNegPairs + m_numUnsegPosPairs) > 0,
                                                             m_stringMetrics[i]);
	m_numActualPosPairs = m_selector.numActualPositives();
	m_numActualNegPairs = m_selector.numActualNegatives();
	...
	for (int j = 0; j < 00; j++) {
	  ...
	  strPairList.add(pair);	  
        ...
        System.out.println("\n" + getTimestamp() + " Starting to train string metric " + i + " on " + strPairList.size() + " string pairs");
	((LearnableStringMetric)m_stringMetrics[i]).trainMetric(strPairList);
      }
    }
    System.out.println(getTimestamp() + " Finished training SumInstanceMetric.");
  }
   ...
   */
  protected ArrayList<String> getStringList(Instances trainData, Instances testData, int attrIdx) {
  ...
  public int getNumPosPairs() { return m_numPosPairs; } 
  ...
  public int getNumNegPairs() { return m_numNegPairs; }

  
  /** Set the number of same-class training pairs
   * @param numUnsegPosPairs the number of same-class training pairs to create for training  */
  public void setNumUnsegPosPairs(int numUnsegPosPairs) {
    m_numUnsegPosPairs = numUnsegPosPairs;
  }
  public int getNumUnsegPosPairs() { return m_numUnsegPosPairs; } 

  
  /** Set the number of different-class training pairs
   * @param numUnsegNegPairs the number of different-class training pairs to create for training
   */
  public void setNumUnsegNegPairs(int numUnsegNegPairs) {
    m_numUnsegNegPairs = numUnsegNegPairs;
  }
  public int getNumUnsegNegPairs() { return m_numUnsegNegPairs; }

  
  /** Set the number of different-class training pairs
   * @param numUnsegNegPairs the number of different-class training pairs to create for training
   */
  public void setNumSegSamePairs(int numSegSamePairs) {
    m_numSegSamePairs = numSegSamePairs;
  }
  public int getNumSegSamePairs() { return m_numSegSamePairs; }
  ...
  public String [] getOptions() {
    ...
    if (m_numUnsegPosPairs > 0) {
      options[current++] = "-pU";
      options[current++] = "" + m_numUnsegPosPairs;
    }
    ...
    if (m_numUnsegNegPairs > 0) {
      options[current++] = "-nU";
      options[current++] = "" + m_numUnsegNegPairs;
    }

    if (m_numSegSamePairs > 0) {
      options[current++] = "-pSS";
      options[current++] = "" + m_numSegSamePairs;
    } 

    options[current++] = "-M";
    options[current++] = Utils.removeSubstring(m_metric.getClass().getName(),
