...
public class ClassifierInstanceMetric extends InstanceMetric implements OptionHandler, Serializable {
  ...
  public ClassifierInstanceMetric() {
	...
	((SMO)m_classifier).setUseRBF(true); 
    }
  } 
  ...
  public void trainInstanceMetric(Instances trainData, Instances testData) throws Exception {
	...
	for (int j = 0; j < m_attrIdxs.length; j++) {
          ...
          for (Object str : attrStringLists[j]) list.add((String)str);
	  ((DataDependentStringMetric)m_fieldMetrics[i][j]).buildMetric(list); 
	}
      }
	...
	String diffName = trainData.relationName() + "." +
	  Utils.removeSubstring(m_fieldMetrics[0].getClass().getName(), "weka.linkage.metrics.");
	...
	eval.crossValidateModel(classifier, m_diffInstances, 5, new Random(0));
   ...
   */
  protected ArrayList getStringList(Instances trainData,
                                    Instances testData, int attrIdx) {
  ...
  public Enumeration listOptions() {
    ...
    newVector.addElement(new Option("\tMetric.\n"
				    +"\t(default=AffineEditDistance)", "M", 1,"-M metric_name metric_options"));
...
//      }
    ...
    System.out.println("Classifier name: " + classifierName +
                       "\nClassifier parameters: " + concatStringArray(classifierSpec));
    setClassifier((Classifier)
                  Classifier.forName(classifierName, classifierSpec));
  }
  ...
  public String [] getOptions() {
    ...
    for (int i = 0; i < m_stringMetrics.length; i++) {
      options[current++] = Utils.removeSubstring(m_stringMetrics[i].getClass().getName(),
    ...
    options[current++] = "-C";
    options[current++] = Utils.removeSubstring(m_classifier.getClass().getName(),
                                               "weka.classifiers.");
