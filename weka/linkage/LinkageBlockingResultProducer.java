...
public class  LinkageBlockingResultProducer 
  ...
  /** Don't dedupe if true */
  protected boolean m_blockingOnly = false;
  ...
  /** Some basic pre-processing on instances:  lower-case, remove punctuation, etc. */
  protected void preprocessInstances() {
    String delimiters = " \t\n\r\f\'\"\\!@#$%^&*()_-+={}<>,.;:|[]{}/*~`";
    ...
    // convert every attribute of every instance
    for (int i = 0; i < numInstances; i++) {
      Instance instance = m_instances.instance(i);

      for (int attrIdx = 0; attrIdx < numAttributes; attrIdx++) {
        if (attrIdx == classIdx) continue; 
        String strVal = instance.stringValue(attrIdx); 
        strVal = strVal.toLowerCase();

        StringTokenizer tokenizer = new StringTokenizer(strVal, delimiters);
        StringBuffer newStrVal = new StringBuffer();

        while (tokenizer.hasMoreTokens()) {
          newStrVal.append(tokenizer.nextToken());
          newStrVal.append(" "); 
        }

        // cut off the trailing space
        if (newStrVal.length() > 0) {
          newStrVal.deleteCharAt(newStrVal.length()-1);
        }

        instance.setValue(attrIdx, newStrVal.toString()); 
      }
    } 
  } 
  ...
  public void setAdditionalMeasures(String [] additionalMeasures) {
    ...
    if (m_splitEvaluator != null) {
      System.err.println(" LinkageBlockingResultProducer: setting additional "
			 +"measures for "
			 +"split evaluator");
  ...
  public double getMeasure(String additionalMeasureName) {
	...
	getMeasure(additionalMeasureName);
      ...
      throw new IllegalArgumentException("LinkageBlockingResultProducer: "
  ...
  protected Object [] processResults(Object[] prResults, double recallLevel) {
    ...
    double maxFM = 0;
    double maxRR = 0;
    Object[] lastResult = (Object[]) prResults[prResults.length-1];
    Object[] results = new Object[lastResult.length]; 
    Arrays.fill(results, new Double(0.0)); 
	...
	results = nextResults;
      ...
      double reductionRatio = ((Double) nextResults[3]).doubleValue();
      if (reductionRatio > maxRR) {
	maxRR = reductionRatio;
	results = nextResults;
      }
      
      double fmeasure = ((Double) nextResults[4]).doubleValue();
      if (fmeasure > maxFM) {
	maxFM = fmeasure;
      }
    }
    ...
    returnResults[3] = new Double(maxRR);
    returnResults[4] = new Double(maxFM);
  ...
  /**
   * Get if Deduper has to be run
   * @return true if Deduper does not have to be run
   */
  public boolean getBlockingOnly() {
    return m_blockingOnly;
  }
  
  /**
   * Set to true if deduper does not have to be run
   * @param d true if deduper does not have to be run
   */
  public void setBlockingOnly(boolean d) {
    m_blockingOnly = d;
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    // SE.
    if (getBlockingOnly()) {
      m_splitEvaluator = new BlockingSplitEvaluator();
      setSplitEvaluator((BlockingSplitEvaluator)Utils.forName(
		      SplitEvaluator.class,
		      seName,
		      null));
    } else {
        m_splitEvaluator = new LinkageSplitEvaluator();
  ...
  public String [] getOptions() {
    ...
    String [] options = new String [seOptions.length + 11];
    int current = 0;
  ...
  public String toString() {
    ...
    String result = "LinkageBlockingResultProducer: ";
  ...
  public static void main(String [] args) {
    LinkageBlockingResultProducer rp = new LinkageBlockingResultProducer();
